...
class TxTennaSegment:
    ...
    def serialize_to_json(self):
        data = {"i": self.payload_id, "t": self.payload}
        ...
        if self.block:
            data = {"h": self.tx_hash, "b": self.block}
    ...
    def deserialize_from_json(cls, json_string):
        ...
        if not cls.segment_json_is_valid(data):
            raise AttributeError(
                "Segment JSON is valid but not properly constructed."
                "Refer to MuleTools documentation for details.\r\n \
                {json_string}"
            )
        ...
        return (
            ...
            and (
                (
                    "s" in data
                    and "h" in data
                    and ("c" not in data or ("c" in data and data["c"] == 0))
                )
                or (
                    "c" in data
                    and data["c"] > 0
            ...
            or ("b" in data and data["b"] >= 0 and "h" in data)
        )

    @classmethod
    def tx_to_segments(
        self, gid, strHexTx, strHexTxHash, messageIdx=0, network="m", isZ85=False
    ):
        #
