...
from lntenna.lightning.lnaddr import (
    shorten_amount,
    unshorten_amount,
    LnAddr,
    lnencode,
...
from lntenna.lightning.bech32 import bech32_encode, bech32_decode
...
def test_roundtrip():
    ...
    tests = [
        ...
        LnAddr(
            RHASH,
            amount=24,
            tags=[("f", "3EktnHQD7RiAE6uzMj2ZifT9YgRrkSgzQX"), ("h", longdescription)],
        ),
        LnAddr(
            RHASH,
            amount=24,
        ...
        LnAddr(
            RHASH, amount=24, tags=[("n", unhexlify(PUBKEY)), ("h", longdescription)]
        ),
    ]
...
def test_n_decoding():
    ...
    hrp, data = bech32_decode(
        lnencode(
            LnAddr(RHASH, amount=24, tags=[("d", ""), ("n", unhexlify(PUBKEY))]),
            PRIVKEY,
        )
    )
    databits = u5_to_bitarray(data)
